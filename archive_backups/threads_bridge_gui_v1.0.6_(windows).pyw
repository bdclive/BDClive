# ====================================================================
# THREADS LAB BRIDGE - NATIVE WINDOWS DESKTOP GUI EDITION (v1.0.6)
# ====================================================================
...
def send_rsvp_card(ev_id, ev_name, date_str, user_count, names_list):
    ...
    payload = {
        "content": "",
        "embeds": [{
            "title": "🎟️ MOVIE NIGHT RSVP",
            "description": f"🎬 **Movie:** {ev_name}\n📅 **Date:** {date_str}  •  👥 **RSVPs:** {user_count} {count_label}\n\n📜 **Confirmed Attendees:**\n{roster_text}",
            "color": 15844367,
            "footer": {
...
def get_facebook_page_insights():
    try:
        r = session.get(f"https://graph.facebook.com/v19.0/{META_PAGE_ID}?fields=followers_count&access_token={META_TOKEN}", timeout=5)
        if r.status_code == 200: return str(r.json().get('followers_count', 0)), "OK"
        try:
            r_alt = session.get(f"https://www.facebook.com/plugins/page.php?href=https%3A%2F%2Fwww.facebook.com%2F{META_PAGE_ID}", timeout=5)
            if r_alt.status_code == 200 and "followers" in r_alt.text:
                import re
                m = re.search(r'([\d,]+)\s+followers', r_alt.text)
...
class BridgeGUIApp:
    def __init__(self, root):
        self.root = root
        self.root.title("🎬 Brian's Theater — Bridge Control Panel v1.0.6")
    ...
    def engine_loop(self):
        ...
        while self.running:
            try:
                    ...
                    if isinstance(guilds, list) and guilds:
                        ...
                        if r_events.status_code == 200:
                            ...
                            if not events:
                                ...
                                date_str, _ = format_event_datetime(next_ev.get('scheduled_start_time'))
                                ...
                                send_rsvp_card(ev_id, ev_name, date_str, len(u_names), u_names)
                ...
                # Push to Firebase
                ok = push_to_firebase(plex_cnt, t_chat, t_view, fb_c, ig_c, th_f, th_v, yt_c, tt_c, x_c, "0", discord_rsvp_val)
