# ====================================================================
# THREADS LAB BRIDGE - NATIVE WINDOWS DESKTOP GUI EDITION (v1.0.40)
# ====================================================================
...
def get_snapchat_data(username="briandivacox"):
    ...
    # 1. Try OAuth Token GraphQL query
    ...
    # 2. Fallback: Web Scraper with Desktop Headers
    try:
...
THEATER_SYNC_URL = "https://livecounters-8eaa8-default-rtdb.firebaseio.com/theaterSync.json"

def push_theater_sync(now_playing="No Movie Playing", next_title="None", next_time="TBD", rsvp_count="0"):
    try:
        ts_payload = {
            "nowPlaying": str(now_playing),
            "nextTitle": str(next_title),
            "nextTime": str(next_time),
            "discordRsvp": str(rsvp_count),
            "timestamp": int(time.time() * 1000)
        }
        session.patch(THEATER_SYNC_URL, json=ts_payload, timeout=3)
    except: pass
...
class BridgeGUIApp:
    def __init__(self, root):
        self.root = root
        self.root.title("🎬 Brian's Theater — Bridge Control Panel v1.0.40")
                    ...
                    if isinstance(guilds, list) and guilds:
                        ...
                        if r_events.status_code == 200:
                            ...
                            if not events:
                                ...
                                push_theater_sync(now_playing="No Movie Playing", next_title=ev_name, next_time=f"{date_str} {time_str}", rsvp_count=len(u_names))
