# ====================================================================
# THREADS LAB BRIDGE - CLI STATIC BOX EDITION (v1.0.40)
# ====================================================================
import requests
import time
import sys
    ...
    try:
        ...
        for g in guilds:
            ...
            if r_events.status_code == 200:
                ...
                push_theater_sync(now_playing="No Movie Playing", next_title=ev_name, next_time=f"{date_str} {time_str}", rsvp_count=user_count)
...
try:
    while True:
        ...
        print("+--------------------------------------------------------------------------------+")
        print(f"| Meta  FB: {fmt_num(cached_fb):<6} | IG: {fmt_num(cached_ig):<6} | Threads: {fmt_num(cached_th_fol)}/{cached_th_views:<12}                |")
        print(f"| Media YT: {fmt_num(cached_yt):<6} | TT: {fmt_num(cached_tt):<6} | X: {fmt_num(cached_x):<7} | Snap: {fmt_num(cached_snap):<6}            |")
        print("+--------------------------------------------------------------------------------+")
